...
def sim_Tanimoto(prefs,p1,p2):
    ...
    print('\n')
    tot = len(prefs[p1]) + len(prefs[p2])

    #if they have no ratings in common, return 0
    if nosimitems == 0 or tot == 0 :
        return 0
    b=nosimitems/tot
    print(b)
    
    return nosimitems/tot
...
# Gets recommendations for a person by using a weighted average
# of every other users rankings
def getRecommendationsFromTopSimilar(prefs,person,num=5,similarity=sim_pearson):
    totals={}
    simSums={}
    topsimilar=topMatches(prefs,person,num)
    for others in topsimilar:
            #dont compare me to myself
            #if others==person : continue
            #sim=similarity(prefs,person,others)
            ...
            #ignore scores of zero or lower
            #if sim<=0 : continue            
            for item in prefs[others[1]]:
                    # only score movies That I have not seen
                    if item not in prefs[person] or prefs[person][item]==0:
                            sim=others[0]
                            #similarity * score
                            totals.setdefault(item,0)
                            totals[item]+=prefs[others[1]][item]*sim
                            #Sum of similarities
                            simSums.setdefault(item,0)
                            simSums[item]+=sim
